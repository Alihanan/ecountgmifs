...
`ecountgmifs` is an R package for fitting extended generalized monotone
...
normalization factor and pass it to `ecountgmifs()`:
...
weight_prior <- ecountgmifs.weight.prior(
  prior = prior_binary,
  eta = c(1, 10, 100),
  label = "Example prior"
)

fit_grid <- ecountgmifs(
  X = X,
  y = y,
